...
- **Tips** — rule-based suggestions for reducing token usage. 19 detectors covering cache-hit rate, repeated file reads, repeated Bash commands, Opus turns that would fit on Sonnet, oversized tool results, subagent outliers, skill-listing budget pressure, oversized `CLAUDE.md`, cross-workspace leaks, dead skills (90d unused), subagent sprawl (sidechain dominates main thread), Bash bloat (commands without output limiters), context-window pressure, repeated identical Bash errors, heavy web-fetch sessions, Opus-only workspaces, MCP-server sprawl, stacked `CLAUDE.md` files, and overlong skill descriptions. Each tip is dismissable for 14 days; drill-down links jump to the responsible session, project, or skill.
